...
class RGCROCOPretrainData(Dataset):
    def __init__(self, config, split='train'):
        ...
        # # RGC
        # self.RGC_data_root = os.path.join('./dataset/pretrain/', split)
        # # self.data_path = os.path.join(self.data_root, split + '_img_idx2path.pkl')
        # self.RGC_data_path = os.path.join(self.RGC_data_root, split + '_img_idx2path_medpix.pkl')
        ...
        # self.RGC_train_json_raw = json.load(open('./dataset/pretrain/train.json'))
        # self.RGC_max_img_id = max([data['img_id'] for data in self.RGC_train_json_raw])
        # self.RGC_max_cap_id = max([data['caps_id'] for data in self.RGC_train_json_raw])
        # ROCO
        ...
        logger.info(f"total training sample number: {self.ROCO_img_num + self.medicat_img_num}")



    def __len__(self):
        return self.ROCO_img_num + self.medicat_img_num

    def get_data_by_idx(self, index):

        if index < self.ROCO_img_num:
            ...
            data = self.medicat_data[index- self.ROCO_img_num]
            id = index
...
def parse_args():
    ...
    parser.add_argument('--conv', help='convolutional layer for images',
                        ...
                        , type=str)
    parser.add_argument('--save_model_name', help='name of model to be saved', type=str, default='swin-roco-medicat')
...
if __name__ == '__main__':
    ...
    logger = setup_logger(__name__, f"log", 0, f"{args.conv}-roco+medicat-{time_tmp.replace(':', '-')}.txt")
    ...
    if not os.path.exists(model_name):
        os.mkdir(model_name)
    pretrain_MVLBert(model, pretrain_dataloader, args.epochs, model_name=model_name, logger=logger, save_freq=30)
